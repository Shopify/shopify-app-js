...
import {mockExternalRequest} from '../../../__test-helpers/request-mock';
import {
  TEST_SHOP,
  API_KEY,
  API_SECRET_KEY,
} from '../../../__test-helpers/const';
...
describe('authenticating flow requests', () => {
  ...
  describe('token refresh for expired offline sessions', () => {
    it('does not refresh token when session is not expired', async () => {
      ...
      // WHEN
      const {session} = await shopify.authenticate.flow(request);
    ...
    it('does not refresh token when distribution is ShopifyAdmin', async () => {
      ...
      const body = {shopify_domain: TEST_SHOP};
