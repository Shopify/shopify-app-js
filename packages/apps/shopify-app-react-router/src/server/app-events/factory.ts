import {GlobalApiToken, HttpResponseError} from '@shopify/shopify-api';
...
export function appEventsFactory(params: BasicParams): AppEvents {
  const {api} = params;
  ...
  return {
    log: async (event) => {
      ...
      try {
        return await api.appEvents.log({
          ...event,
