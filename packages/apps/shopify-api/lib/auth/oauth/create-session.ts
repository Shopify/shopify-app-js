...
export function createSession({
  ...
  const getOnlineSessionProperties = (responseBody: OnlineAccessResponse) => {
    const {access_token, scope, refresh_token, refresh_token_expires_in, ...rest} = responseBody;
    ...
    return {
      ...
      ...(refresh_token_expires_in && {refreshTokenExpires: getSessionExpiration(refresh_token_expires_in)}),
    };
  };
