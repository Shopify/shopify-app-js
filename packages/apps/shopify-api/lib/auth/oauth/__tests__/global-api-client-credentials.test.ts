...
describe('globalApiClientCredentials', () => {
  ...
  test('uses the fallback expiry for an opaque token without response metadata', async () => {
    ...
    expect(token.expiresAt.getTime()).toBeWithinSecondsOf(expectedExpiration, 1);
  ...
  test('uses response expiry metadata for opaque tokens', async () => {
    ...
    queueMockResponse(
      JSON.stringify({access_token: 'opaque', expires_in: 120, scope: 'read write'}),
    );
  ...
  test('prefers response expiry and scopes over JWT metadata', async () => {
    ...
    queueMockResponse(
      JSON.stringify({access_token: accessToken, expires_in: 1, scope: 'response_scope'}),
    );
  ...
  test('mints a new token on every call', async () => {
    const shopify = shopifyApi(testConfig());
    queueMockResponse(JSON.stringify({access_token: 'token-1', expires_in: 3600}));
    queueMockResponse(JSON.stringify({access_token: 'token-2', expires_in: 3600}));

    const first = await shopify.auth.globalApiClientCredentials();
    const second = await shopify.auth.globalApiClientCredentials();

    expect(second.token.accessToken).not.toBe(first.token.accessToken);
  ...
  test('includes the OAuth error and description in the thrown message', async () => {
    ...
    queueMockResponse(
      JSON.stringify({
        ...
        error_description: 'Could not find Shopify API application with client_id "test_key".',
      }),
      {statusCode: 400, statusText: 'Bad Request'},
    );

    const error = await shopify.auth.globalApiClientCredentials().catch((thrown) => thrown);
  ...
  test('omits token request bodies from debug logs without changing the wire body', async () => {
    ...
    const accessToken = fakeGlobalToken({exp: Math.floor(Date.now() / 1000) + 3600});
    queueMockResponse(JSON.stringify({access_token: accessToken}));
  ...
  test('throws HttpResponseError when token minting fails', async () => {
    const shopify = shopifyApi(testConfig());
    queueMockResponse(
      JSON.stringify({error: 'application_cannot_be_found', error_description: 'Application cannot be found'}),
      {statusCode: 400, statusText: 'Bad Request'},
    );
  ...
  test('posts to a configured Global API URL', async () => {
    const shopify = shopifyApi(testConfig({globalApiUrl: 'https://api.my-spin.shopify.io'}));
    const token = fakeGlobalToken({exp: Math.floor(Date.now() / 1000) + 3600});
    queueMockResponse(JSON.stringify({access_token: token}));
