...
function eventMock(
  ...
  return {
    request: new Request(EVENTS_URL, {method: 'POST', body: EVENT_BODY}),
...
function eventAuthorization(callIndex: number) {
  return (
    fetchMock.mock.calls[callIndex][1]!.headers as Record<string, string>
  ).Authorization;
}

describe('appEventsFactory', () => {
  it('mints a Global API token and posts the event', async () => {
    ...
    expect(eventAuthorization(1)).toBe('Bearer global-token-1');
  ...
  it('reuses the cached token for a second event', async () => {
    ...
    expect(eventAuthorization(2)).toBe('Bearer global-token-1');
  });
  ...
  it('coalesces concurrent mints', async () => {
    ...
    await Promise.all([shopify.appEvents.log(event), shopify.appEvents.log(event)]);
  ...
  it('mints a new token and retries once after a 401', async () => {
    ...
    expect(eventAuthorization(3)).toBe('Bearer global-token-2');
  });
