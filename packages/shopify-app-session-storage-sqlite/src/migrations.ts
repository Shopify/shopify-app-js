...
async function migrateToV1_0_1(engine: SqliteEngine): Promise<void> {
  ...
  const rename = `
    ALTER TABLE ${engine.sessionPersistenceIdentifier} RENAME TO ${tempTableName};
  `;
  await engine.query(rename);

  // 2. Create new table with 1024 chars
  const newTable = `
        CREATE TABLE ${engine.sessionPersistenceIdentifier} (
  ...
  const insert = `
    INSERT INTO ${engine.sessionPersistenceIdentifier} (id,shop,state,isOnline,expires,scope,accessToken,onlineAccessInfo)
