...
export async function verifyRequest(app: any) {
  return async (req:any, res: any, next: any) => {
    ...
    const authPath = `${app.mountpath}/auth`;
    ...
    if (session?.isActive()) {
      try {
        if (CONFIG.billing.required) {
          ...
          if (!hasPayment) {
            return returnTopLevelRedirection(req, res, next, confirmationUrl as string);
          }
        } else {
    ...
    return returnTopLevelRedirection(req, res, next, `${authPath}?shop=${shop}`);
  };
}
