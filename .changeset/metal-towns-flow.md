...
- **Internal Helpers**:
  - Introduced `getOfflineSession`: A unified helper to load an offline session and ensure it's valid (refreshing if necessary).
  - Added `ensureOfflineTokenIsNotExpired`: Logic to check expiration and trigger refresh.
  - Added `refreshToken` helper: Wraps the API client's refresh token functionality.
